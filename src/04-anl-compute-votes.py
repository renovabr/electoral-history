...
def main(argv):
    ...
    for st in STATES:
        ...
        df3 = pd.merge(df0, df2, on='sq_candidate', how='inner')

        df3 = df3.drop_duplicates()
        final = df3.sort_values(by=['qt_votes_nominal'], inplace=False, ascending=False)

        # final.to_csv('test.csv', index=False, sep=",")
        print('Inserting the data in table: votes_computed')
        final.to_sql('votes_computed', con=engine, if_exists='replace', 
